...
async function buscarTempo() {
    ...
    if (tempo) {
        ...
        <p><strong>Temperatura:</strong> ${tempo.temp_min}°C - 
        ${tempo.temp_max}°C</p>
        ...
        <p><strong>Por do Sol:</strong> ${tempo.sunset} </p>
        <p><strong>Latitude:</strong> ${tempo.lat} <strong/> longitude:</strong> 
        ${tempo.lon} </p>
     `;
        resultadoDiv.style.display = "block";
        } else {
            resultadoDiv.innerHTML = "<p>Erro ao buscar dados. Verifique o nome da cidade seu burro, aprende a escrever ❤😊❤.</p>";
            resultadoDiv.style.display = "block";
        }
    }
...
async function getPrevisao(cidade) {
    ...
    const url = `https://api.openweathermap.org/data/2.5/wearther?=${cidade}
    ...
    try {
        ...
        const surise = new Date(dados.sys.sunrise * 1000).toLocaleTimeString();
